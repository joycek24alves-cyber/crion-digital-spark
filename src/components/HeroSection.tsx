...
const HeroSection = () => (
  <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-main noise-overlay">
    ...
    <div className="absolute inset-0 pointer-events-none opacity-[0.02]" style={{
      backgroundImage: "linear-gradient(hsl(263 100% 59% / 0.4) 1px, transparent 1px), linear-gradient(90deg, hsl(263 100% 59% / 0.4) 1px, transparent 1px)",
    ...
    <div className="relative z-10 container mx-auto px-4 text-center max-w-5xl">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="mb-8">
        <span className="font-heading text-base sm:text-lg tracking-[0.5em] uppercase font-bold" style={{ color: "#7B2EFF", textShadow: "0 0 25px hsl(263 100% 59% / 0.6), 0 0 80px hsl(263 100% 59% / 0.2)" }}>
          CRION DIGITAL
        </span>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.25 }} className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-5 py-2 mb-12 backdrop-blur-sm">
        <span className="h-2 w-2 rounded-full animate-pulse_dot" style={{ backgroundColor: "#7B2EFF", boxShadow: "0 0 10px #7B2EFF" }} />
        <span className="font-body text-sm font-medium text-primary/90">Vagas limitadas este mês</span>
      </motion.div>

      {/* Headline */}
      <motion.h1 initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="font-heading text-5xl sm:text-6xl md:text-7xl lg:text-[5.5rem] xl:text-[7rem] leading-[0.88] mb-2 uppercase">
        <span className="block text-foreground" style={{ textShadow: "0 0 60px hsl(263 100% 59% / 0.08)" }}>
          Clique não paga conta.
        </span>
        <span className="block text-gradient-intense mt-3" style={{ filter: "drop-shadow(0 0 40px hsl(263 100% 59% / 0.4))" }}>
          Venda sim.
        </span>
      </motion.h1>

      {/* Divider */}
      <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.9 }} className="w-32 h-[2px] mx-auto my-10" style={{ background: "linear-gradient(90deg, transparent, #7B2EFF, #00D4FF, transparent)", boxShadow: "0 0 20px hsl(263 100% 59% / 0.4), 0 0 60px hsl(187 100% 50% / 0.1)" }} />
